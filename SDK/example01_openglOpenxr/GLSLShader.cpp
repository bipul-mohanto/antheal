...
{
	std::string vt = FS_ReadAllText((std::string("C:/Users/local-admin/Desktop/openxrOptix/example01_openglOpenxr/shaders/") + vert + ".vertex.glsl").c_str());
	if (vt.size() == 0) {
		Log("shader: %s.vs not found\n", vert);
		return -1;
	}
	std::string ft = FS_ReadAllText((std::string("C:/Users/local-admin/Desktop/openxrOptix/example01_openglOpenxr/shaders/") + frag + ".fragment.glsl").c_str());
	if (ft.size() == 0) {
